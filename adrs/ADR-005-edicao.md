...
| Teste 6 | Tema por token CSS, preflight do Tailwind, `prefers-reduced-motion` | Arquitetura base exige cores só por token e dois temas | Cor e fundo da área editável batem com `var(--foreground)`/`var(--background)` no tema claro e no escuro. O app alterna por `.dark` no elemento raiz (`src/styles.css`, `@custom-variant dark`), não pelas classes `.theme-*` que `insumos/BASE.md` declarava. Nenhuma animação acima de 0,01 ms com `reducedMotion: reduce` |
