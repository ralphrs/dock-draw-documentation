// Roda as 30 fixtures do ADR 002 contra a implementação de referência.
// Uso: npm install && npm run check  (a partir desta pasta; fixtures em ../fixtures)
import { readFileSync, existsSync } from 'node:fs'
import { join, dirname } from 'node:path'
import { fileURLToPath } from 'node:url'
import { normalize, validate } from './dokmd.mjs'
...
for (const f of manifest) {
  ...
  // Idempotência para qualquer entrada: N(N(x)) === N(x)
  const n1 = normalize(input)
  checks.push(['idempotente', normalize(n1) === n1])

  if (expected !== null) {
    checks.push(['expected é ponto fixo', normalize(expected) === expected])
    const d = validate(expected)
    ...
    const codes = [...new Set(validate(n1).map((x) => x.code).filter((c) => c.startsWith('DOK-E')))].sort()
