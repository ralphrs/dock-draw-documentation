...
14. **O corpus de 30 fixtures não cobre `list.spread = true`.** Varredura das 30 fixtures pelo `parseDok`: a fixture 06 tem `listItem.spread = true` e item de vários blocos, e nenhuma tem `list.spread = true`, a linha em branco entre itens irmãos. O MDXEditor passa na 06 e falha nessa forma (`adrs/_work/spike-s1/extra/x05-ul-ul-varios-blocos.md` e `x06-ol-ol-varios-blocos.md`, evidência em `RESULTADO-S1.md`). Um editor pode passar em 30/30 e ainda perder a forma da lista. Dono: ADR 002, fatia F5 (fixtures), que ganha um caso com lista frouxa na raiz. Só aviso de cobertura, não muda decisão nem número do S-1.
