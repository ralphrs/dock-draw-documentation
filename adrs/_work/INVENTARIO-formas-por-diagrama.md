...
### Primeira lista da `DEC-0030`, 26 marcas (`DDP-387`)

A `DEC-0030` estendeu a regra de ícone oficial (`DEC-0023`) de nuvem para toda marca de tecnologia. As 26 marcas abaixo, pesquisa em 2026-09-21 (cinco agentes em paralelo, um por grupo). Oito já tinham fonte registrada em ficha anterior, sem repetir a pesquisa: `Kubernetes`, `Docker`, `PostgreSQL`, `Python` (`FICHA-INVENTARIO-icones-cncf.md` e `FICHA-INVENTARIO-icones-linguagens-infra.md`), `Jenkins`, `GitHub`, `GitLab` (`FICHA-INVENTARIO-tipo-cicd.md`) e `Argo` (mesmo regime `cncf/artwork` do Kubernetes, sem página própria).

**Ficam sob o regime `cncf/artwork` mais Linux Foundation Trademark Usage, mesma conclusão do Kubernetes.**

| Marca | Fonte do arquivo | Página de marca própria | Veredito |
| --- | --- | --- | --- |
| Helm | [`cncf/artwork/projects/helm`](https://github.com/cncf/artwork/tree/main/projects/helm), SVG/PNG | Nenhuma. [`helm.sh/community/art`](https://helm.sh/community/art/) remete direto à Linux Foundation | Bloqueado sem autorização escrita, mesma cadeia do Kubernetes |
| Prometheus | [`cncf/artwork/projects/prometheus`](https://github.com/cncf/artwork/tree/main/projects/prometheus), SVG/PNG | Nenhuma. FAQ oficial cita só a nota padrão da Linux Foundation. Pedido de "logo pack" próprio aberto desde 2016 (`prometheus/docs#413`), nunca atendido | Bloqueado, mesma cadeia do Kubernetes |
| Istio | [`cncf/artwork/projects/istio`](https://github.com/cncf/artwork/tree/main/projects/istio), SVG/PNG | Sim, própria e mais restritiva: [`istio/community/BRANDING.md`](https://github.com/istio/community/blob/master/BRANDING.md): "Don't use the Istio mark prominently on posters, brochures, signs, websites, or other marketing materials to promote your events, products or services without written permission from Istio Steering." Histórico de marca dividido entre a Open Usage Commons (2020) e a entrada na CNCF (2022), sem página unificada vigente em istio.io | Bloqueado, com barra explícita a material de marketing/produto |

**Fica no padrão Docker (proibição por padrão, exceção estreita só de uso nominativo).**

| Marca | Fonte oficial | Formato | Citação literal | Veredito |
| --- | --- | --- | --- | --- |
| MySQL | [mysql.com/about/legal/trademark.html](https://www.mysql.com/about/legal/trademark.html), badges condicionais em `/about/legal/logos.html` | PNG (badges) | "no Oracle or MySQL Conditional Use Logo may be used on any product packaging or documentation" | Bloqueado para uso amplo, só badge condicional específica sem autorização |
| Apache Kafka | [apache.org/foundation/marks](https://www.apache.org/foundation/marks/) (geral ASF) + [kafka.apache.org/community/trademark](https://kafka.apache.org/community/trademark/) | PNG em `apache.org/logos/res/kafka/kafka_highres.png`, sem SVG oficial achado | "In general you may not use ASF trademarks in any software product branding" | Nome livre para uso nominativo, logo vedado como branding de produto |
| RabbitMQ | [rabbitmq.com/trademark-guidelines.html](https://www.rabbitmq.com/trademark-guidelines.html) | SVG oficial hospedado na própria página | "You may engage in 'nominative use' of the RabbitMQ name, but this does not allow you to use the logo" [...] uso do logo "reserved solely for use by Broadcom in its unaltered form" | Arquivo disponível não implica licença de uso do logo |
| Redis | [redis.io/legal/trademark-guidelines](https://redis.io/legal/trademark-guidelines/) | Nenhum brand kit oficial achado, só terceiros | Isenção de licença só para "open source Redis modules, clients or any non-commercial project", com atribuição textual obrigatória | DokDraw (SaaS comercial) fica fora da isenção estreita |
| Nginx | [f5.com/company/policies/trademarks](https://www.f5.com/company/policies/trademarks) | Nenhum brand kit oficial achado, `nginx.com/trademarks/` redireciona a página de produto | "No right, license or permission is granted to use F5's trademarks except as may be provided explicitly by F5" | Bloqueado, sem página de marca específica do NGINX |
| Terraform | [hashicorp.com/en/trademark-policy](https://www.hashicorp.com/en/trademark-policy) | ZIP oficial em `hashicorp.com/en/brand/hcp-product-logos`, "Please don't modify these" | Permissão sem autorização só "to identify and hyperlink to main page" do site do projeto, condição que uma forma de diagrama não cumpre | Bloqueado para o caso do DokDraw |
| Ansible | [redhat.com/.../trademark-guidelines-and-policies](https://www.redhat.com/en/about/trademark-guidelines-and-policies) (geral) + [`ansible/logos`](https://github.com/ansible/logos) (comunitário, escopo estreito) | Repositório comunitário sem arquivo de licença formal, README cita CC BY-SA 4.0 em texto corrido | "Any use of the Ansible logos and marks without Ansible approval is prohibited" | Bloqueado. O `TRADEMARKS.md` do repositório cobre só subprojetos (AWX etc.), não o logo "A" que uma forma do DokDraw representaria |
| Grafana | [grafana.com/trademark-policy](https://grafana.com/trademark-policy/) | Nenhum brand kit oficial, ícone existe no repositório de código (AGPLv3, copyleft forte) | "Do not use the Grafana Labs Marks in any commercial or marketing context other than as expressly permitted in this policy unless you have obtained explicit written permission" | Bloqueado sem autorização escrita |

**Casos mistos ou atípicos, um por marca.**

- **MongoDB.** [mongodb.com/legal/trademark-usage-guidelines](https://www.mongodb.com/legal/trademark-usage-guidelines). O único da amostra mais perto do padrão PostgreSQL: "it is OK if you describe, 'services for MongoDB'", sem exigência textual de autorização prévia para uso nominativo básico, mas proíbe qualquer alteração: "we like them how they are". SVG oficial em `mongodb.com/company/newsroom/brand-resources`, mídia completa via Brandfolder.
- **Java (Oracle).** Duas imagens com regras opostas, achado central: o mascote Duke é BSD, livre para uso e modificação, ficha em [dev.java/duke](https://dev.java/duke). Já a "Coffee Cup with Steam", a xícara que normalmente se reconhece como logo do Java, "should only be used under a written, commercial trademark license" (`Java Branding and Licensing Guidelines`, PDF versão 21, dezembro de 2024, [oracle.com/a/ocom/docs/java-licensing-logo-guidelines-1908204.pdf](https://www.oracle.com/a/ocom/docs/java-licensing-logo-guidelines-1908204.pdf)).
- **JavaScript.** Caso sem paralelo nos outros 25: a marca "JavaScript" pertence à Oracle (herdada da Sun/Netscape), sem nenhuma política de logo publicada pela Oracle para o nome. Está sob disputa pública ativa: a Deno formalizou pedido de cancelamento no USPTO em 2024-11-22 ([deno.com/blog/deno-v-oracle](https://deno.com/blog/deno-v-oracle)). A logo amarela "JS" que o ecossistema inteiro usa não é oficial: criação comunitária de Chris Williams (2011), projeto `logo.js`, licença MIT, sem qualquer chancela da titular da marca.
- **TypeScript (Microsoft).** [typescriptlang.org/branding](https://www.typescriptlang.org/branding/), pacote de SVG oficial baixável. Meio-termo: "Please use the blue TypeScript mark above as the primary logo for TypeScript [...] for blog posts, news articles". Proibido sem contato: "Use the TypeScript logos for your application/product."
- **Go (Google).** Duas camadas com regras opostas, igual ao caso Java. O mascote gopher, de Renee French, é CC BY 4.0 (a página oficial [go.dev/brand](https://go.dev/brand) cita 4.0 hoje, não a 3.0 histórica de 2009). Já a "Go Logo" (o wordmark estilizado) exige aprovação prévia fora do uso nominativo em texto simples: "Do not use the Go Logo or Go as a stylized form without permission."
- **Node.js (OpenJS Foundation).** Marca transferida da Joyent para a OpenJS Foundation em fevereiro de 2022. Política em [trademark-policy.openjsf.org](https://trademark-policy.openjsf.org), adotada 2021-01-12. SVG oficial em `nodejs.org/en/about/branding`. Fair use nominativo com frase padrão liberada ("`<produto>` for Node.js"), mas "Do not use a logo of the OpenJS Foundation on posters, brochures, signs, websites [...] without written permission".
- **React (em transição de titular).** Histórico: Meta Platforms, com [Meta Open Source Trademark Policy](https://opensource.fb.com/legal/trademark/). Mudança relevante para a data desta pesquisa: a Meta anunciou em 2025-10-07 a criação da React Foundation sob a Linux Foundation, com transferência efetiva de React, React Native e JSX em 2026-02-24 ([react.dev/blog/2025/10/07/introducing-the-react-foundation](https://react.dev/blog/2025/10/07/introducing-the-react-foundation)). Nenhuma política de marca própria da React Foundation foi encontrada publicada ainda. Pela política da Meta, ainda o texto disponível: "all other uses of the OS Trademarks are prohibited without written permission from Meta". Nenhuma página de brand kit oficial encontrada em react.dev.

Todas as marcas sem SVG oficial redistribuível (Nginx, Redis, Grafana, RabbitMQ com ressalva, Ansible) têm entrada equivalente em `simple-icons` (CC0-1.0), mesma fonte de arquivo já adotada para a amostra de dez fornecedores da seção anterior. O eixo de licença do arquivo SVG e o eixo de marca registrada continuam separados em todos os 26 casos, sem exceção: nenhum arquivo redistribuível sob CC0/MIT resolve sozinho a autorização de marca para uso comercial.

**Recomendação:** nenhuma das 26 marcas fica no regime permissivo da AWS (`DEC-0023`). MongoDB é a mais próxima de um uso liberado sem autorização prévia, e mesmo essa exige logo intacto. As demais 25 exigem autorização escrita para o caso específico do DokDraw (forma de diagrama redistribuída numa biblioteca de produto comercial), estão bloqueadas por página de marca própria mais restritiva que o padrão da Linux Foundation (Istio), ou têm zona de uso liberada condicionada a algo que uma forma de diagrama não cumpre (hiperlink ao site do projeto, no caso Terraform). **Custo aceito:** a lista de 26 não fecha o inventário todo de marcas que `DEC-0030` abriu, só a primeira lista pedida; decisão de uso (aguardar autorização por fornecedor, tratar cada caso como a AWS e aceitar risco, ou não incluir a marca) fica com o humano, marca por marca.
...
**Pacote e versão, registrados em 2026-09-21 (`DDP-384`, `DEC-0030`).** Google Cloud: baixado de [cloud.google.com/icons](https://cloud.google.com/icons), pacotes `category-icons.zip` e `core-products-icons.zip` (mais `google-cloud-legacy-icons.zip`, ícones antigos que a própria página diz "should not be used as of 2026"). Nenhum arquivo tem número de versão no nome nem no cabeçalho HTTP (`HEAD` devolve 405, método não permitido pelo servidor). A única referência de data é a capa do guia que acompanha o pacote, `google-cloud-product-icons.pdf`: "Updated: May 2026". O termo de uso é o já citado acima, a `<meta name="description">` da página, sem cláusula de licença própria.

OCI: baixado de [docs.oracle.com/en-us/iaas/Content/General/Reference/graphicsfordiagrams.htm](https://docs.oracle.com/en-us/iaas/Content/General/Reference/graphicsfordiagrams.htm), título da página "OCI Architecture Diagram Toolkits". Três pacotes: `OCI_Icons.pptx`, `OCI-Style-Guide-for-Drawio.zip`, `OCI_Icons_Visio.zip`. Nenhum tem versão no nome; o único dado de data é o rodapé da página, "Copyright © 2026, Oracle and/or its affiliates." Citação literal do único texto de uso na página: "Use these assets to draw custom architecture diagrams for your OCI implementation. Each of these formats contains OCI service icons and templates where possible." Sem cláusula de licença, uso comercial, alteração ou atribuição nesta página. As diretrizes gerais de marca da Oracle ([oracle.com/legal/trademarks](https://www.oracle.com/legal/trademarks/), consultadas na mesma data) distinguem uso nominativo/descritivo da marca, permitido sem autorização ("works with Oracle software"), do uso do logotipo em si, que pede autorização escrita à parte ("Third Party Usage Guidelines for Oracle Logos"). Nenhuma das duas páginas de marca cita ícone de arquitetura por nome.

Nenhuma proibição escrita e explícita de uso em ferramenta de diagrama apareceu para Google Cloud nem para OCI. O risco que a `DEC-0030` já registra como aceito pelo humano continua o mesmo, sem novidade que o mude.
