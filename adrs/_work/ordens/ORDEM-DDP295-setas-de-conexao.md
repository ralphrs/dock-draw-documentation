...
**3. Clicar numa seta clona na direção dela.** Sem elemento nenhum naquela direção, o clique cria uma cópia do elemento de origem (mesmo tipo, nome, tecnologia, descrição, estilo, largura e altura, no molde do que `copyNode` já copia), posicionada a uma distância fixa na direção da seta, sem sobrepor o original, e já conectada ao original pela mesma relação que `connectElements` cria.

**4. Clicar numa seta com elemento já naquela direção conecta a ele.** Em vez de clonar, o clique cria a relação entre o elemento de origem e o elemento existente na direção da seta, sem criar elemento novo. "Naquela direção" significa o elemento mais próximo cujo centro cai no semiplano daquele lado, dentro de uma faixa razoável de alinhamento, não qualquer elemento distante no mesmo eixo.
