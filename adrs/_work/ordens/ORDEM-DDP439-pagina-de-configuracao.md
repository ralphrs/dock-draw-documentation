...
**5. A tela de cabeçalho e rodapé vira filha do layout.** `projetos.$projectId.configuracoes.cabecalho-rodape.tsx` passa a renderizar só o conteúdo: sai o bloco das linhas 333 a 345 (abertura do `AppShell` até o fechamento do `header`) e sai o `</AppShell>` do fim do arquivo (linha 715). Fica o `div` de `flex min-h-0 flex-1` com as abas, as zonas e a prévia. O `input` de arquivo continua. Nada mais muda nessa tela.
