...
**2. Moldura, em componente novo `aws-icon-node.tsx` seguindo `image-node.tsx`.** Quadrado com fundo `var(--card)`, borda `var(--border)`, o SVG do serviço centralizado dentro (URL assinada pelo mecanismo do item 6) e o nome oficial do serviço como rótulo abaixo da moldura, truncado como `ElementLabels` já faz.

**3. Lacuna declarada de geometria.** A `DDP-237` deixa em aberto "qual variante (serviço x recurso) vira padrão e o tamanho do ícone dentro da moldura", a resolver no ADR-015. Esta ordem não inventa esses dois números. Até o ADR-015 fechar, só a variante `service` entra na paleta, com a moldura no tamanho mínimo das outras formas (120×60, ajustado a proporção quadrada dentro desse mínimo) e o ícone no miolo com a margem que `ImageNodeBody` já usa. A sessão C confere contra o frame `cWI2iIHQDBPPKqSu8J06oS` (página Moldura de serviço) e pode pedir ajuste de padding sem reabrir esta ordem.

**4. Seis tipos de contêiner, mesmo arquivo de tipos.** `aws_region`, `aws_vpc`, `aws_az`, `aws_subnet_public`, `aws_subnet_private`, `aws_security_group`. Todos com `shape: "aws-boundary"` novo em `catalog.ts`, cantos retos (`rx=0`, diferente do `group` genérico, arredondado: a `DDP-239` confirma que canto reto é o jeito da AWS). Tabela de estilo, literal dos achados aceitos na `DDP-239`/`DDP-240` (cor e traço são exceção declarada ao token único, como a `DDP-239` já documentou para o ícone de grupo: são cores oficiais da AWS, não do DokDraw):
...
**9. Exportação `.drawio`, `drawioStyle` em `export-diagram.tsx`.** Estilos conferidos na paleta "AWS / Groups" do draw.io (`jgraph/drawio`, `src/main/webapp/js/diagramly/sidebar/Sidebar-AWS4.js`, linhas 257 a 305, commit `f3abfe0f`, consultado em 2026-09-22). Prefixo comum dos grupos, chamado `G` abaixo: `outlineConnect=0;gradientColor=none;html=1;whiteSpace=wrap;fontSize=12;fontStyle=0;container=1;pointerEvents=0;collapsible=0;recursiveResize=0;shape=mxgraph.aws4.group;verticalAlign=top;align=left;spacingLeft=30;`.
