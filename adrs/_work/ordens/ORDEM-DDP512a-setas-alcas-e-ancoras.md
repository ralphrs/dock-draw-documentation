...
**7. Roteamento ortogonal por lado.** Reescrever `orthogonalize` em `edge-path.ts` com as âncoras: o primeiro segmento sai perpendicular ao lado de origem por 20px, o último entra perpendicular ao lado de destino por 20px. Entre os dois trechos: lados opostos horizontais (`l`/`r`) quebram em `midX`, lados verticais (`t`/`b`) em `midY`, lados mistos fazem um único cotovelo em `(fim.x, início.y)` quando a origem é `l`/`r` e `(início.x, fim.y)` quando é `t`/`b`. Mesmo lado nas duas pontas (`r` e `r`, `l` e `l`, `t` e `t`, `b` e `b`): a rota sai, contorna e entra pelo mesmo lado, com um trilho a 20px além do ponto mais externo. Para `r`, `X = max(início.x, fim.x) + 20` e os pontos são início, `(X, início.y)`, `(X, fim.y)`, fim. Para `l`, `X = min(início.x, fim.x) - 20`. Para `b`, `Y = max(início.y, fim.y) + 20` com `(início.x, Y)` e `(fim.x, Y)`. Para `t`, `Y = min(início.y, fim.y) - 20`. Quando os dois nós se sobrepõem no eixo do trilho, o trilho passa 20px além do nó mais externo, não do ponto. Sem âncora, o lado é o que `borderPoint` toca. Com waypoints, cada waypoint é respeitado como hoje. O raio de canto continua o de `buildPath`.
...
| `public.relationships` é do ADR 001, mutável, com `style_props` Json sem contrato de banco | Âncoras gravadas ali, sem migração. O ADR 015 pode mover para coluna própria depois, com custo declarado |
