...
`src/domain/c4/families.ts`, linha 38, marca `basicas` como `available: false` com `groups: []`, e a janela Mais formas mostra "Básicas" como "em breve". As quatro formas têm design aprovado. Esta ordem só liga o que já foi decidido.
...
**5. Alça de conexão do triângulo, em `src/components/editor/c4-node.tsx`.** As alças hoje (linhas 117 a 157) ancoram na caixa, o que já serve para elipse, losango e texto solto: nenhuma mudança nessas três. Só o triângulo (ápice para cima) diverge nas laterais. Com os pontos de conexão da `DDP-515` (três por lado em `t` 0,25, 0,5 e 0,75, mais os cantos), para `meta.shape === "triangle"` a posição de cada ponto dos lados `l` e `r` sai da aresta inclinada: no lado `l`, `x = (w / 2) * (1 - t)` e `y = t * h`, e no lado `r`, `x = (w / 2) * (1 + t)` e `y = t * h`, com `t` contado de cima para baixo. Em `t` 0,5 isso dá `w / 4` e `3w / 4`, a regra da `DDP-437`. Os cantos de cima (`tl`, `tr`) do triângulo não existem: só o ápice `t:0.5` fica no topo. O lado `t` fica só com o ápice, e o lado `b` com os três pontos e os dois cantos de baixo. `anchorPoint` da `DDP-515` recebe a forma e devolve o ponto do contorno, que a aresta usa. `meta` já existe no componente (linha 53).
...
**8. Exportação draw.io, `src/components/editor/export-diagram.tsx`, função `drawioStyle` (linhas 499 a 510, `base` na 500).** SVG e PNG não mudam: `buildSvg` (linha 218) desenha por `ElementShape`/`ElementLabels` e herda o item 3. Só o `.drawio` monta estilo próprio. Quatro ramos novos antes do `return` padrão da linha 509:
...
O ramo `text` não usa `base`: a forma não tem preenchimento nem contorno. `ellipse`, `rhombus` e `triangle` do mxGraph já conectam pelo ápice e pelo meio das arestas, sem `exitX`/`exitY` nas arestas.
...
- Não mexer em `palette-item.tsx` nem em `familias-de-formas.tsx`: a miniatura em escala (`DDP-445`) já usa `ElementShape` e herda as formas novas.
- Não mudar `NodeResizer` em `c4-node.tsx` além do item 5: contorno e alças de seleção já vêm do resizer padrão (linhas 68 a 81).
...
- **Paleta `ntconsult` sem cor aprovada.** A `DDP-250` só cobriu DokDraw e C4 Padrão. Na paleta `ntconsult` (`styles.css`, linhas 521 a 570) os neutros são de marca e não servem de base. As três variáveis ficam sem regra em `[data-palette="ntconsult"]` até a sessão D propor. O Lovable declara isso no resultado, sem escolher cor.
- **Fórmula de truncamento do losango.** A `DDP-260` aprovou o resultado (14 caracteres em 160×100), não a fórmula. O item 4 deriva a fórmula do triângulo (`DDP-437`), conferida só nesse ponto. A sessão C confere no preview em outros tamanhos.
