...
**6. Cursor, Esc e clique no vazio.** Enquanto `connectArmed || connectFrom`, o contêiner do quadro (onde `ReactFlow` renderiza) ganha `cursor-crosshair` ou equivalente, trocando de volta ao cursor normal quando o modo desliga. No `useEffect` de atalhos de teclado já existente na rota, tecla `Escape` com `connectArmed || connectFrom` verdadeiro zera os dois, cancelando a qualquer momento, sem criar relação. Clique em área vazia do quadro faz o mesmo que o Esc: `onPaneClick` zera `connectArmed` e `connectFrom` antes de limpar a seleção.
...
**8. Convivência com a seleção múltipla (`DDP-301`, já no app).** Coberto em parte pelo item 4: o clique em elemento com o modo de conexão ligado nunca chega à lógica de seleção. Falta a caixa de arrasto e o Ctrl+A, e não existe hoje nenhuma condição que os suspenda: crie uma. No `ReactFlow` de `diagram-canvas.tsx`, `selectionOnDrag` passa a ser `!(connectArmed || connectFrom)`, e o atalho Ctrl+A da rota não faz nada enquanto `connectArmed || connectFrom` for verdadeiro.
...
**1. Fica só o botão Baixar do cabeçalho.** O `DropdownMenu` de `projetos.$projectId.diagramas.$viewId.tsx` não muda, com as três opções de hoje (PNG, SVG, .drawio).

**2. Sai a seção Baixar do painel de detalhes.** Remove o bloco `<div className="space-y-1"><Label>Baixar</Label>...</div>` , os três `Button` de Imagem PNG, Vetor SVG e Arquivo .drawio.

**3. Saem os três itens de exportar do menu de clique direito do quadro.** Em `diagram-canvas.tsx`, `paneEntries`: remove os itens (`Baixar PNG`, `Baixar SVG`, `Baixar .drawio`) e o separador logo antes deles, que fica sem função depois da remoção, porque nada mais segue ele na lista.
