...
**1. Seleção por anel, não por troca de cor.** Em `element-shape.tsx`, `stroke` passa a ser sempre `line` e `strokeWidth` sempre `s.strokeWidth`. A seleção vira um anel separado, desenhado depois da forma dentro do mesmo `<g>`: o contorno externo da forma, `fill="none"`, `stroke="var(--ring)"`, `strokeWidth 2.5`, `vectorEffect="non-scaling-stroke"`, deslocado 3px para fora com `transform` de escala em torno do centro. Para não redesenhar os detalhes decorativos, `renderShape` ganha um parâmetro `outlineOnly?: boolean`: quando verdadeiro, cilindro (linhas 125 a 142) e tubo (144 a 161) não desenham o arco de acabamento em `textColor`, navegador (173 a 203) não desenha os pontinhos nem a barra de endereço, e terminal (205 a 233) não desenha o ícone de prompt. Retângulo e elipse podem usar um `rect`/`ellipse` 3px maior em vez do `transform`. Sem `pointer-events`. O halo (`filter` da linha 38) continua. A exportação não desenha o anel. Em `c4-edge.tsx`: `color = sp.color || "var(--edge)"`, e a seleção passa a ser um segundo `<path>` por baixo, mesmo `d`, `stroke="var(--ring)"`, largura `sp.width + 6`, opacidade 0,35. A espessura da linha selecionada volta a ser `sp.width`.
...
**3. Delete apaga tudo que está selecionado.** A rota passa a guardar `selectedRelIds: string[]` no lugar de `selectedRelId`. Pontos de uso a migrar, todos na rota: a definição (linha 316), o derivado `selectedRel` (linha 402, que vira o único item quando a lista tem um só e `null` nos outros casos), e as chamadas de `setSelectedRelId` nas linhas 1002, 1009, 1222, 1889, 1901, 1908, 2045 e 3191. As linhas 1002 e 1009 trocam o id provisório pelo id real só quando o atual bate com o provisório: em lista, mapear o item (`ids.map((c) => (c === relId ? rel.id : c))`) e, na falha, filtrar o item, sem substituir a lista inteira. `handleSelectionChange` (`diagram-canvas.tsx`, linhas 252 a 261) passa também `edges` para a rota, e `rfEdges` marca `selected` pela lista. Clique numa conexão seleciona só ela. Shift+clique acrescenta ou tira: `onEdgeClick` (`diagram-canvas.tsx`, linha 515) lê `event.shiftKey` e chama `onToggleRelationship(edge.id)` em vez de `onSelectRelationship`, sem limpar os nós selecionados. O laço de seleção pega nós e conexões dentro da área, pelo `edges` que o React Flow já devolve em `onSelectionChange`. `handleDeleteSelection` apaga nós e conexões selecionados num único item de histórico (Ctrl+Z restaura os dois). Delete e Backspace valem para forma, imagem colada (`image-node.tsx`) e conexão. O painel Detalhes mostra "CONEXÃO" quando há uma só conexão selecionada e "SELEÇÃO (n)" quando há mais de um item. O teclado continua ignorando o evento quando o foco está em `INPUT`, `TEXTAREA` ou `contentEditable`.
