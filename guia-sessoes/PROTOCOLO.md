...
- **A sessão A nunca põe nem deixa uma issue num status que a própria JQL dela vigia.** Ao terminar de tratar uma issue, ela vai para onde o próximo passo de fato está: `CONCLUÍDA` se acabou, `EM ANDAMENTO` se outra sessão continua, `AGUARDANDO APROVAÇÃO` se voltou a depender do humano. A escuta usa status como sinal de que outra sessão ou o humano agiu. Status posto pela sessão A é sinal falso, e o custo é um ciclo inteiro de escuta gasto com trabalho que ela mesma acabou de fazer. Aconteceu três vezes em 2026-09-20: a ordem da fatia F1 deixada em `EM REVISÃO` depois de lida, a issue de revisão do dono do produto criada em `EM ANDAMENTO`, e uma aprovação respondida em parte deixada em `EM ANDAMENTO` enquanto esperava o resto da resposta.
...
- **Todo bloco de código da ordem vai entre `{code}` e `{code}`.** O campo de descrição da API v2 do Jira é wiki markup, não Markdown. Texto colado cru é interpretado: `(n)` e `(x)` viram emoticon, `-texto-` vira tachado, `|` vira separador de tabela, `?texto?` vira citação. A gravação fica íntegra, e quem lê pelo ADF ou por markdown recebe o texto já deturpado. `{code}` desliga a interpretação no trecho. Achado na execução da fatia F1 (`DDP-74`), onde a ordem chegou ao executor com regex, união de tipos e parâmetros corrompidos.
...
- **A tabela de restrições só aceita linha que corresponda a um texto de `restricoes_impostas` do `LEDGER.md`.** Propriedade que o autor considera boa, mas que nenhum contrato impõe, não entra: infla o que parece vinculante e produz linha que promete verificação impossível. Se a propriedade tiver valor, ela sobe como proposta de restrição nova, com aprovação do humano, e só depois vira linha de tabela. A ordem da fatia F3 trouxe "não importa biblioteca de render", que nenhum mecanismo podia reprovar porque nenhum contrato exigia.
