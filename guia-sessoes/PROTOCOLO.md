...
- **O Lovable confere só no projeto de teste.** Toda ordem diz, na seção de conferência, que o preview roda no projeto `Regressão` (o mesmo de `guia-sessoes/TESTE-REGRESSIVO.md`), nunca num projeto real do humano. Em 2026-09-22 a `DDP-516` foi conferida apagando e desfazendo quatro formas do projeto NTConsult. A mensagem de `send_message` repete a regra em uma linha.
- **Texto de tela sem referência interna (`DEC-0041`).** Nenhum rótulo, selo, aviso, toast ou mensagem que a pessoa vê cita tarefa, ADR, DEC, ordem, sessão ou Lovable. Recurso que ainda não existe ganha aviso na linguagem da pessoa ("Convites por projeto em breve"). Toda ordem leva a linha "Nenhum texto de tela cita tarefa, ADR, ordem ou sessão" em "O que não fazer". C confere na revisão da ordem e na da execução. A varre os textos do commit por `ADR`, `DDP-`, `DEC-`, `Lovable` e `sessão` antes de abrir o card `app-release`.
