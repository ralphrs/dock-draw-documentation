...
**Todo arquivo é criado no plano `SquadPro`.** Pegue o `planKey` dele na resposta do `whoami` e use esse plano em todo `create_new_file`. Não crie nada em rascunho pessoal nem em outro plano ou equipe. Dentro do `SquadPro`, todo arquivo do DokDraw fica no projeto **`dok draw app`**. Arquivo fora dessa pasta é arquivo perdido para o humano. A pasta tem duas subpastas: **`draft`**, com o arquivo de trabalho onde toda forma nasce, e **`done`**, com a biblioteca que só recebe forma aprovada pelo humano. A biblioteca é uma por família de formas, com o nome "DokDraw - Formas <família> Aprovadas": "DokDraw - Formas C4 Model Aprovadas", "DokDraw - Formas AWS Aprovadas", "DokDraw - Formas Básicas Aprovadas". Se o `SquadPro` não aparecer no `whoami`, **pare** e avise o humano, sem criar arquivo em lugar nenhum.
